...
namespace com.onlineobject.objectnet.embedded {
    /// <summary>The send mode of a <see cref="EmbeddedMessage"/>.</summary>
    public enum MessageSendMode : byte
    {
        /// <summary>Guarantees order but not delivery. Notifies the sender of what happened via the <see cref="EmbeddedConnection.NotifyDelivered"/> and <see cref="EmbeddedConnection.NotifyLost"/>
        /// events. The receiver must handle notify messages via the <see cref="EmbeddedConnection.NotifyReceived"/> event, <i>which is different from the other two send modes</i>.</summary>
    ...
    public class EmbeddedMessage
    {
        ...
        private const int BitsPerByte = EmbeddedConverter.BitsPerByte;
        /// <summary>The number of bits in each data segment.</summary>
        private const int BitsPerSegment = EmbeddedConverter.BitsPerULong;
        ...
        public static int MaxPayloadSize
        {
            ...
            set
            {
                if (EmbeddedPeer.ActiveCount > 0)
                    throw new InvalidOperationException($"Changing the '{nameof(MaxPayloadSize)}' is not allowed while a {nameof(EmbeddedServer)} or {nameof(EmbeddedClient)} is running!");
                ...
                EmbeddedPendingMessage.ClearPool();
            }
        }
        ...
        /// <summary>How many messages to add to the pool for each <see cref="EmbeddedServer"/> or <see cref="EmbeddedClient"/> instance that is started.</summary>
        /// <remarks>Changes will not affect <see cref="EmbeddedServer"/> and <see cref="EmbeddedClient"/> instances which are already running until they are restarted.</remarks>
        public static byte InstancesPerPeer { get; set; } = 4;
        /// <summary>A pool of reusable message instances.</summary>
        private static readonly List<EmbeddedMessage> pool = new List<EmbeddedMessage>(InstancesPerPeer * 2);
        ...
        /// <summary>Initializes a reusable <see cref="EmbeddedMessage"/> instance.</summary>
        private EmbeddedMessage() => data = new ulong[maxArraySize];
        ...
        {
            EmbeddedMessage message = RetrieveFromPool();
            message.readBit = 0;
            message.writeBit = 0;
            return message;
        }
        ...
        public static EmbeddedMessage Create(MessageSendMode sendMode)
        {
        ...
        /// <returns>A message instance ready to be sent.</returns>
        public static EmbeddedMessage Create(MessageSendMode sendMode, ushort id)
        {
        ...
        public static EmbeddedMessage Create(MessageSendMode sendMode, Enum id)
        {
        ...
        /// <returns>A message instance ready to be sent.</returns>
        internal static EmbeddedMessage Create(MessageHeader header)
        {
            return RetrieveFromPool().Init(header);
        }

        #region Pooling
        /// <summary>Trims the message pool to a more appropriate size for how many <see cref="EmbeddedServer"/> and/or <see cref="EmbeddedClient"/> instances are currently running.</summary>
        public static void TrimPool()
        {
            ...
            else
            {
                ...
                int idealInstanceAmount = EmbeddedPeer.ActiveCount * InstancesPerPeer;
                if (pool.Count > idealInstanceAmount)
                {
                    pool.RemoveRange(EmbeddedPeer.ActiveCount * InstancesPerPeer, pool.Count - idealInstanceAmount);
        ...
        private static EmbeddedMessage RetrieveFromPool()
        {
        ...
        private EmbeddedMessage Init(MessageHeader header)
        {
            data[0] = (byte)header;
            SetHeader(header);
            return this;
        ...
        internal EmbeddedMessage Init(byte firstByte, int contentLength, out MessageHeader header)
        {
        ...
        public EmbeddedMessage AddMessage(EmbeddedMessage message) => AddMessage(message, message.UnreadBits, message.readBit);
        ...
        /// <returns>The message that the bits were added to.</returns>
        /// <remarks>This method does not move <paramref name="message"/>'s internal read position!</remarks>
        public EmbeddedMessage AddMessage(EmbeddedMessage message, int amount, int startBit)
        {
            if (UnwrittenBits < amount)
                throw new InsufficientCapacityException(this, nameof(EmbeddedMessage), amount);
        ...
        public EmbeddedMessage ReserveBits(int amount)
        {
            if (UnwrittenBits < amount)
        ...
        /// <returns>The message instance.</returns>
        public EmbeddedMessage SkipBits(int amount)
        {
            if (UnreadBits < amount)
                EmbeddedLogger.Log(LogType.Error, $"Message only contains {UnreadBits} unread {EmbeddedHelper.CorrectForm(UnreadBits, "bit")}, which is not enough to skip {amount}!");
        ...
        public EmbeddedMessage SetBits(ulong bitfield, int amount, int startBit)
        {
            ...
            EmbeddedConverter.SetBits(bitfield, amount, data, startBit);
            return this;
        }
        ...
        public EmbeddedMessage PeekBits(int amount, int startBit, out byte bitfield)
        {
            ...
            EmbeddedConverter.GetBits(amount, data, startBit, out bitfield);
            return this;
        }
        ...
        public EmbeddedMessage PeekBits(int amount, int startBit, out ushort bitfield)
        {
        ...
        /// <inheritdoc cref="PeekBits(int, int, out byte)"/>
        public EmbeddedMessage PeekBits(int amount, int startBit, out uint bitfield)
        {
        ...
        /// <inheritdoc cref="PeekBits(int, int, out byte)"/>
        public EmbeddedMessage PeekBits(int amount, int startBit, out ulong bitfield)
        {
            if (amount > sizeof(ulong) * BitsPerByte)
        ...
        /// <param name="amount">The number of bits to add.</param>
        /// <returns>The message that the bits were added to.</returns>
        public EmbeddedMessage AddBits(byte bitfield, int amount)
        {
            if (amount > BitsPerByte)
            ...
            EmbeddedConverter.ByteToBits(bitfield, data, writeBit);
            writeBit += amount;
            return this;
        }
        /// <summary>Adds up to 16 of the given bits to the message.</summary>
        /// <inheritdoc cref="AddBits(byte, int)"/>
        public EmbeddedMessage AddBits(ushort bitfield, int amount)
        {
            if (amount > sizeof(ushort) * BitsPerByte)
            ...
            EmbeddedConverter.UShortToBits(bitfield, data, writeBit);
            writeBit += amount;
            return this;
        }
        /// <summary>Adds up to 32 of the given bits to the message.</summary>
        /// <inheritdoc cref="AddBits(byte, int)"/>
        public EmbeddedMessage AddBits(uint bitfield, int amount)
        {
            if (amount > sizeof(uint) * BitsPerByte)
            ...
            EmbeddedConverter.UIntToBits(bitfield, data, writeBit);
            writeBit += amount;
            return this;
        }
        /// <summary>Adds up to 64 of the given bits to the message.</summary>
        /// <inheritdoc cref="AddBits(byte, int)"/>
        public EmbeddedMessage AddBits(ulong bitfield, int amount)
        {
            if (amount > sizeof(ulong) * BitsPerByte)
            ...
            EmbeddedConverter.ULongToBits(bitfield, data, writeBit);
            writeBit += amount;
            return this;
        }
        ...
        public EmbeddedMessage GetBits(int amount, out byte bitfield)
        {
        ...
        public EmbeddedMessage GetBits(int amount, out ushort bitfield)
        {
            PeekBits(amount, readBit, out bitfield);
            readBit += amount;
            return this;
        ...
        /// <inheritdoc cref="GetBits(int, out byte)"/>
        public EmbeddedMessage GetBits(int amount, out uint bitfield)
        {
            PeekBits(amount, readBit, out bitfield);
            readBit += amount;
            return this;
        ...
        /// <inheritdoc cref="GetBits(int, out byte)"/>
        public EmbeddedMessage GetBits(int amount, out ulong bitfield)
        {
            PeekBits(amount, readBit, out bitfield);
            readBit += amount;
            return this;
        ...
        public EmbeddedMessage AddVarLong(long value) => AddVarULong((ulong)EmbeddedConverter.ZigZagEncode(value));
        ...
        public EmbeddedMessage AddVarULong(ulong value)
        {
            do
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public long GetVarLong() => EmbeddedConverter.ZigZagDecode((long)GetVarULong());
        ...
        public EmbeddedMessage AddSByte(sbyte value)
        {
            if (UnwrittenBits < BitsPerByte)
                throw new InsufficientCapacityException(this, SByteName, BitsPerByte);

            EmbeddedConverter.SByteToBits(value, data, writeBit);
            writeBit += BitsPerByte;
            return this;
        }
        ...
        public byte GetByte()
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(ByteName, $"{default(byte)}"));
                return default;
            }

            byte value = EmbeddedConverter.ByteFromBits(data, readBit);
            readBit += BitsPerByte;
            return value;
        }
        ...
        public sbyte GetSByte()
        {
            if (UnreadBits < BitsPerByte)
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(SByteName, $"{default(sbyte)}"));
                return default;
            }

            sbyte value = EmbeddedConverter.SByteFromBits(data, readBit);
            readBit += BitsPerByte;
            return value;
        }
        ...
        public EmbeddedMessage AddBytes(byte[] array, bool includeLength = true)
        {
            if (includeLength)
            ...
            else
            {
                ...
                {
                    EmbeddedConverter.ByteToBits(array[i], data, writeBit);
                    writeBit += BitsPerByte;
                }
            }
        ...
        /// <param name="includeLength">Whether or not to include the length of the array in the message.</param>
        /// <returns>The message that the array was added to.</returns>
        public EmbeddedMessage AddSBytes(sbyte[] array, bool includeLength = true)
        {
            if (includeLength)
                AddVarULong((uint)array.Length);
            ...
            for (int i = 0; i < array.Length; i++)
            {
                EmbeddedConverter.SByteToBits(array[i], data, writeBit);
                writeBit += BitsPerByte;
            }
        ...
        private void ReadBytes(int amount, byte[] intoArray, int startIndex = 0)
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(amount, ByteName));
            ...
            else
            {
                ...
                {
                    intoArray[startIndex + i] = EmbeddedConverter.ByteFromBits(data, readBit);
                    readBit += BitsPerByte;
                }
            }
        }
        ...
        private void ReadSBytes(int amount, sbyte[] intoArray, int startIndex = 0)
        {
            if (UnreadBits < amount * BitsPerByte)
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(amount, SByteName));
                amount = UnreadBits / BitsPerByte;
            }

            for (int i = 0; i < amount; i++)
            {
                intoArray[startIndex + i] = EmbeddedConverter.SByteFromBits(data, readBit);
                readBit += BitsPerByte;
            }
        }
        #endregion
        ...
        public EmbeddedMessage AddBool(bool value)
        {
            if (UnwrittenBits < 1)
                throw new InsufficientCapacityException(this, BoolName, 1);

            EmbeddedConverter.BoolToBit(value, data, writeBit++);
            return this;
        }
        ...
        public bool GetBool()
        {
            if (UnreadBits < 1)
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(BoolName, $"{default(bool)}"));
                return default;
            }

            return EmbeddedConverter.BoolFromBit(data, readBit++);
        }
        ...
        /// <param name="includeLength">Whether or not to include the length of the array in the message.</param>
        /// <returns>The message that the array was added to.</returns>
        public EmbeddedMessage AddBools(bool[] array, bool includeLength = true)
        {
            if (includeLength)
                AddVarULong((uint)array.Length);
            ...
            for (int i = 0; i < array.Length; i++)
                EmbeddedConverter.BoolToBit(array[i], data, writeBit++);
        ...
        private void ReadBools(int amount, bool[] intoArray, int startIndex = 0)
        {
            if (UnreadBits < amount)
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(amount, BoolName));
                amount = UnreadBits;
            }

            for (int i = 0; i < amount; i++)
                intoArray[startIndex + i] = EmbeddedConverter.BoolFromBit(data, readBit++);
        }
        #endregion
        ...
        public EmbeddedMessage AddUShort(ushort value)
        {
            ...
            EmbeddedConverter.UShortToBits(value, data, writeBit);
        ...
        public short GetShort()
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(ShortName, $"{default(short)}"));
                return default;
            }

            short value = EmbeddedConverter.ShortFromBits(data, readBit);
        ...
        public ushort GetUShort()
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(UShortName, $"{default(ushort)}"));
                return default;
            }

            ushort value = EmbeddedConverter.UShortFromBits(data, readBit);
        ...
        /// <param name="includeLength">Whether or not to include the length of the array in the message.</param>
        /// <returns>The message that the array was added to.</returns>
        public EmbeddedMessage AddShorts(short[] array, bool includeLength = true)
        {
            if (includeLength)
                AddVarULong((uint)array.Length);
            ...
            for (int i = 0; i < array.Length; i++)
            {
                EmbeddedConverter.ShortToBits(array[i], data, writeBit);
                writeBit += sizeof(short) * BitsPerByte;
            }
        ...
        /// <param name="includeLength">Whether or not to include the length of the array in the message.</param>
        /// <returns>The message that the array was added to.</returns>
        public EmbeddedMessage AddUShorts(ushort[] array, bool includeLength = true)
        {
            if (includeLength)
                AddVarULong((uint)array.Length);
            ...
            for (int i = 0; i < array.Length; i++)
            {
                EmbeddedConverter.UShortToBits(array[i], data, writeBit);
                writeBit += sizeof(ushort) * BitsPerByte;
            }
        ...
        private void ReadShorts(int amount, short[] intoArray, int startIndex = 0)
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(amount, ShortName));
                amount = UnreadBits / (sizeof(short) * BitsPerByte);
            }

            for (int i = 0; i < amount; i++)
            {
                intoArray[startIndex + i] = EmbeddedConverter.ShortFromBits(data, readBit);
                readBit += sizeof(short) * BitsPerByte;
            }
        }
        ...
        private void ReadUShorts(int amount, ushort[] intoArray, int startIndex = 0)
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(amount, UShortName));
                amount = UnreadBits / (sizeof(ushort) * BitsPerByte);
            }

            for (int i = 0; i < amount; i++)
            {
                intoArray[startIndex + i] = EmbeddedConverter.UShortFromBits(data, readBit);
                readBit += sizeof(ushort) * BitsPerByte;
            }
        }
        #endregion
        ...
        public int GetInt()
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(IntName, $"{default(int)}"));
                return default;
            }

            int value = EmbeddedConverter.IntFromBits(data, readBit);
        ...
        public uint GetUInt()
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(UIntName, $"{default(uint)}"));
                return default;
            }

            uint value = EmbeddedConverter.UIntFromBits(data, readBit);
        ...
        /// <param name="includeLength">Whether or not to include the length of the array in the message.</param>
        /// <returns>The message that the array was added to.</returns>
        public EmbeddedMessage AddInts(int[] array, bool includeLength = true)
        {
            if (includeLength)
                AddVarULong((uint)array.Length);
            ...
            for (int i = 0; i < array.Length; i++)
            {
                EmbeddedConverter.IntToBits(array[i], data, writeBit);
                writeBit += sizeof(int) * BitsPerByte;
            }
        ...
        /// <param name="includeLength">Whether or not to include the length of the array in the message.</param>
        /// <returns>The message that the array was added to.</returns>
        public EmbeddedMessage AddUInts(uint[] array, bool includeLength = true)
        {
            if (includeLength)
                AddVarULong((uint)array.Length);
            ...
            for (int i = 0; i < array.Length; i++)
            {
                EmbeddedConverter.UIntToBits(array[i], data, writeBit);
                writeBit += sizeof(uint) * BitsPerByte;
            }
        ...
        private void ReadInts(int amount, int[] intoArray, int startIndex = 0)
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(amount, IntName));
                amount = UnreadBits / (sizeof(int) * BitsPerByte);
            }

            for (int i = 0; i < amount; i++)
            {
                intoArray[startIndex + i] = EmbeddedConverter.IntFromBits(data, readBit);
                readBit += sizeof(int) * BitsPerByte;
            }
        }
        ...
        private void ReadUInts(int amount, uint[] intoArray, int startIndex = 0)
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(amount, UIntName));
                amount = UnreadBits / (sizeof(uint) * BitsPerByte);
            }

            for (int i = 0; i < amount; i++)
            {
                intoArray[startIndex + i] = EmbeddedConverter.UIntFromBits(data, readBit);
                readBit += sizeof(uint) * BitsPerByte;
            }
        }
        #endregion
        ...
        public long GetLong()
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(LongName, $"{default(long)}"));
                return default;
            }

            long value = EmbeddedConverter.LongFromBits(data, readBit);
        ...
        public ulong GetULong()
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(ULongName, $"{default(ulong)}"));
                return default;
            }

            ulong value = EmbeddedConverter.ULongFromBits(data, readBit);
        ...
        /// <param name="includeLength">Whether or not to include the length of the array in the message.</param>
        /// <returns>The message that the array was added to.</returns>
        public EmbeddedMessage AddLongs(long[] array, bool includeLength = true)
        {
            if (includeLength)
                AddVarULong((uint)array.Length);
            ...
            for (int i = 0; i < array.Length; i++)
            {
                EmbeddedConverter.LongToBits(array[i], data, writeBit);
                writeBit += sizeof(long) * BitsPerByte;
            }
        ...
        /// <param name="includeLength">Whether or not to include the length of the array in the message.</param>
        /// <returns>The message that the array was added to.</returns>
        public EmbeddedMessage AddULongs(ulong[] array, bool includeLength = true)
        {
            if (includeLength)
                AddVarULong((uint)array.Length);
            ...
            for (int i = 0; i < array.Length; i++)
            {
                EmbeddedConverter.ULongToBits(array[i], data, writeBit);
                writeBit += sizeof(ulong) * BitsPerByte;
            }
        ...
        private void ReadLongs(int amount, long[] intoArray, int startIndex = 0)
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(amount, LongName));
                amount = UnreadBits / (sizeof(long) * BitsPerByte);
            }

            for (int i = 0; i < amount; i++)
            {
                intoArray[startIndex + i] = EmbeddedConverter.LongFromBits(data, readBit);
                readBit += sizeof(long) * BitsPerByte;
            }
        }
        ...
        private void ReadULongs(int amount, ulong[] intoArray, int startIndex = 0)
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(amount, ULongName));
                amount = UnreadBits / (sizeof(ulong) * BitsPerByte);
            }

            for (int i = 0; i < amount; i++)
            {
                intoArray[startIndex + i] = EmbeddedConverter.ULongFromBits(data, readBit);
                readBit += sizeof(ulong) * BitsPerByte;
            }
        }
        #endregion
        ...
        public float GetFloat()
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(FloatName, $"{default(float)}"));
                return default;
            }

            float value = EmbeddedConverter.FloatFromBits(data, readBit);
        ...
        /// <param name="includeLength">Whether or not to include the length of the array in the message.</param>
        /// <returns>The message that the array was added to.</returns>
        public EmbeddedMessage AddFloats(float[] array, bool includeLength = true)
        {
            if (includeLength)
                AddVarULong((uint)array.Length);
            ...
            for (int i = 0; i < array.Length; i++)
            {
                EmbeddedConverter.FloatToBits(array[i], data, writeBit);
                writeBit += sizeof(float) * BitsPerByte;
            }
        ...
        private void ReadFloats(int amount, float[] intoArray, int startIndex = 0)
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(amount, FloatName));
                amount = UnreadBits / (sizeof(float) * BitsPerByte);
            }

            for (int i = 0; i < amount; i++)
            {
                intoArray[startIndex + i] = EmbeddedConverter.FloatFromBits(data, readBit);
                readBit += sizeof(float) * BitsPerByte;
            }
        }
        #endregion
        ...
        public EmbeddedMessage AddDouble(double value)
        {
            ...
            EmbeddedConverter.DoubleToBits(value, data, writeBit);
        ...
        public double GetDouble()
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(DoubleName, $"{default(double)}"));
                return default;
            }

            double value = EmbeddedConverter.DoubleFromBits(data, readBit);
        ...
        /// <param name="includeLength">Whether or not to include the length of the array in the message.</param>
        /// <returns>The message that the array was added to.</returns>
        public EmbeddedMessage AddDoubles(double[] array, bool includeLength = true)
        {
            if (includeLength)
                AddVarULong((uint)array.Length);
            ...
            for (int i = 0; i < array.Length; i++)
            {
                EmbeddedConverter.DoubleToBits(array[i], data, writeBit);
                writeBit += sizeof(double) * BitsPerByte;
            }
        ...
        private void ReadDoubles(int amount, double[] intoArray, int startIndex = 0)
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(amount, DoubleName));
                amount = UnreadBits / (sizeof(double) * BitsPerByte);
            }

            for (int i = 0; i < amount; i++)
            {
                intoArray[startIndex + i] = EmbeddedConverter.DoubleFromBits(data, readBit);
                readBit += sizeof(double) * BitsPerByte;
            }
        }
        #endregion
        ...
        public EmbeddedMessage AddString(string value)
        {
        ...
        public string GetString()
        {
            ...
            {
                EmbeddedLogger.Log(LogType.Error, NotEnoughBitsError(StringName, "shortened string"));
        ...
        /// <param name="includeLength">Whether or not to include the length of the array in the message.</param>
        /// <returns>The message that the array was added to.</returns>
        public EmbeddedMessage AddStrings(string[] array, bool includeLength = true)
        {
            if (includeLength)
                AddVarULong((uint)array.Length);
        ...
        public EmbeddedMessage AddSerializable<T>(T value) where T : IEmbeddedMessageSerializable
        {
        ...
        public T GetSerializable<T>() where T : IEmbeddedMessageSerializable, new()
        {
            T t = new T();
            t.Deserialize(this);
            return t;
        ...
        /// <param name="includeLength">Whether or not to include the length of the array in the message.</param>
        /// <returns>The message that the array was added to.</returns>
        public EmbeddedMessage AddSerializables<T>(T[] array, bool includeLength = true) where T : IEmbeddedMessageSerializable
        {
            if (includeLength)
                AddVarULong((uint)array.Length);
        ...
        /// <returns>The array that was retrieved.</returns>
        public T[] GetSerializables<T>() where T : IEmbeddedMessageSerializable, new() => GetSerializables<T>((int)GetVarULong());
        /// <summary>Retrieves an array of serializables from the message.</summary>
        /// <param name="amount">The amount of serializables to retrieve.</param>
        /// <returns>The array that was retrieved.</returns>
        public T[] GetSerializables<T>(int amount) where T : IEmbeddedMessageSerializable, new()
        {
        ...
        /// <param name="intoArray">The array to populate.</param>
        /// <param name="startIndex">The position at which to start populating the array.</param>
        public void GetSerializables<T>(T[] intoArray, int startIndex = 0) where T : IEmbeddedMessageSerializable, new() => GetSerializables<T>((int)GetVarULong(), intoArray, startIndex);
        /// <summary>Populates an array of serializables retrieved from the message.</summary>
        /// <param name="amount">The amount of serializables to retrieve.</param>
        /// <param name="intoArray">The array to populate.</param>
        /// <param name="startIndex">The position at which to start populating the array.</param>
        public void GetSerializables<T>(int amount, T[] intoArray, int startIndex = 0) where T : IEmbeddedMessageSerializable, new()
        {
            if (startIndex + amount > intoArray.Length)
        ...
        private void ReadSerializables<T>(int amount, T[] intoArray, int startIndex = 0) where T : IEmbeddedMessageSerializable, new()
        {
            for (int i = 0; i < amount; i++)
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(byte value) => AddByte(value);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(short value) => AddShort(value);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(ulong value) => AddULong(value);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(float value) => AddFloat(value);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(double value) => AddDouble(value);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(string value) => AddString(value);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add<T>(T value) where T : IEmbeddedMessageSerializable => AddSerializable(value);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(byte[] array, bool includeLength = true) => AddBytes(array, includeLength);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(sbyte[] array, bool includeLength = true) => AddSBytes(array, includeLength);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(bool[] array, bool includeLength = true) => AddBools(array, includeLength);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(short[] array, bool includeLength = true) => AddShorts(array, includeLength);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(ushort[] array, bool includeLength = true) => AddUShorts(array, includeLength);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(int[] array, bool includeLength = true) => AddInts(array, includeLength);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(uint[] array, bool includeLength = true) => AddUInts(array, includeLength);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(long[] array, bool includeLength = true) => AddLongs(array, includeLength);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(ulong[] array, bool includeLength = true) => AddULongs(array, includeLength);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(float[] array, bool includeLength = true) => AddFloats(array, includeLength);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(double[] array, bool includeLength = true) => AddDoubles(array, includeLength);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add(string[] array, bool includeLength = true) => AddStrings(array, includeLength);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EmbeddedMessage Add<T>(T[] array, bool includeLength = true) where T : IEmbeddedMessageSerializable, new() => AddSerializables(array, includeLength);
        #endregion
        #endregion
        ...
        {
            return $"Message only contains {UnreadBits} unread {EmbeddedHelper.CorrectForm(UnreadBits, "bit")}, which is not enough to retrieve a value of type '{valueName}'! Returning {defaultReturn}.";
        }
        ...
        {
            return $"Message only contains {UnreadBits} unread {EmbeddedHelper.CorrectForm(UnreadBits, "bit")}, which is not enough to retrieve {arrayLength} {EmbeddedHelper.CorrectForm(arrayLength, valueName)}! Returned array will contain default elements.";
        }
