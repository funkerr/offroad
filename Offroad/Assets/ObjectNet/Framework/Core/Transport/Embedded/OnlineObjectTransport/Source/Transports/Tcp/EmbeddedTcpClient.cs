...
namespace com.onlineobject.objectnet.embedded.Transports.Tcp {
    /// <summary>A client which can connect to a <see cref="EmbeddedTcpServer"/>.</summary>
    public class EmbeddedTcpClient : EmbeddedTcpPeer, IEmbeddedClient
    {
        /// <inheritdoc/>
        ...
        private EmbeddedTcpConnection tcpConnection;
        ...
        public bool Connect(string hostAddress, out EmbeddedConnection connection, out string connectError)
        {
            ...
            catch (SocketException)
            {
                ...
                // call OnConnected(), and let transport system detect that no connection was established.
            }

            connection = tcpConnection = new EmbeddedTcpConnection(socket, remoteEndPoint, this);
            OnConnected();
            return true;
        }
