...
namespace com.onlineobject.objectnet.embedded.Transports.Tcp {
    /// <summary>A server which can accept connections from <see cref="EmbeddedTcpClient"/>s.</summary>
    public class EmbeddedTcpServer : EmbeddedTcpPeer, IEmbeddedServer
    {
        /// <inheritdoc/>
        ...
        private Dictionary<IPEndPoint, EmbeddedTcpConnection> connections;
        ...
        /// <inheritdoc/>
        public EmbeddedTcpServer(int socketBufferSize = DefaultSocketBufferSize) : this(IPAddress.IPv6Any, socketBufferSize) { }
        ...
        public EmbeddedTcpServer(IPAddress listenAddress, int socketBufferSize = DefaultSocketBufferSize) : base(socketBufferSize)
        {
        ...
        {
            Port = port;
            connections = new Dictionary<IPEndPoint, EmbeddedTcpConnection>();
        ...
        public void Poll()
        {
            ...
            Accept();
            foreach (EmbeddedTcpConnection connection in connections.Values)
                connection.Receive();
        ...
        private void Accept()
        {
            if (socket.Poll(0, SelectMode.SelectRead))
            {
                ...
                {
                    EmbeddedTcpConnection newConnection = new EmbeddedTcpConnection(acceptedSocket, fromEndPoint, this);
        ...
        {
            if (connection is EmbeddedTcpConnection tcpConnection)
            {
        ...
        /// <inheritdoc/>
        protected internal override void OnDataReceived(int amount, EmbeddedTcpConnection fromConnection)
        {
            if ((MessageHeader)(ReceiveBuffer[0] & EmbeddedMessage.HeaderBitmask) == MessageHeader.Connect)
            {
