...
namespace com.onlineobject.objectnet.embedded.Transports.Udp {
    /// <summary>A client which can connect to a <see cref="EmbeddedUdpServer"/>.</summary>
    public class EmbeddedUdpClient : EmbeddedUdpPeer, IEmbeddedClient
    {
        /// <inheritdoc/>
        ...
        private EmbeddedUdpConnection udpConnection;

        /// <inheritdoc/>
        public EmbeddedUdpClient(SocketMode mode = SocketMode.Both, int socketBufferSize = DefaultSocketBufferSize) : base(mode, socketBufferSize) { }
        ...
        public bool Connect(string hostAddress, out EmbeddedConnection connection, out string connectError)
        {
            ...
            connection = udpConnection = new EmbeddedUdpConnection(new IPEndPoint(mode == SocketMode.IPv4Only ? ip : ip.MapToIPv6(), port), this);
