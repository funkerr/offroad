...
namespace com.onlineobject.objectnet.embedded {
    /// <summary>A client that can connect to a <see cref="EmbeddedServer"/>.</summary>
    public class EmbeddedClient : EmbeddedPeer
    {
        ...
        /// <inheritdoc cref="EmbeddedConnection.RTT"/>
        public short RTT => connection.RTT;
        /// <inheritdoc cref="EmbeddedConnection.SmoothRTT"/>
        ...
        /// <summary>Sets the client's <see cref="EmbeddedConnection.TimeoutTime"/>.</summary>
        ...
        public EmbeddedConnection Connection => connection;
        /// <summary>Encapsulates a method that handles a message from a server.</summary>
        /// <param name="message">The message that was received.</param>
        public delegate void MessageHandler(EmbeddedMessage message);
        ...
        public EmbeddedClient(IEmbeddedClient transport, string logName = "CLIENT") : base(logName)
        {
            this.transport = transport;
        }
        /// <summary>Handles initial setup using the built-in UDP transport.</summary>
        /// <param name="logName">The name to use when logging messages via <see cref="EmbeddedLogger"/>.</param>
        public EmbeddedClient(string logName = "CLIENT") : this(new Transports.Udp.EmbeddedUdpClient(), logName) { }
        ...
        /// <remarks>This method does not automatically reconnect to the server. To continue communicating with the server, <see cref="Connect(string, int, byte, EmbeddedMessage, bool)"/> must be called again.</remarks>
        ...
        /// <param name="message">Data that should be sent to the server with the connection attempt. Use <see cref="EmbeddedMessage.Create()"/> to get an empty message instance.</param>
        /// <param name="useMessageHandlers">Whether or not the client should use the built-in message handler system.</param>
        /// <remarks>
        ///   <para>ObjectNet default transport expects the host address to consist of an IP and port, separated by a colon. For example: <c>127.0.0.1:7777</c>. If you are using a different transport, check the relevant documentation for what information it requires in the host address.</para>
        ///   <para>Setting <paramref name="useMessageHandlers"/> to <see langword="false"/> will disable the automatic detection and execution of methods with the <see cref="EmbeddedMessageHandlerAttribute"/>, which is beneficial if you prefer to handle messages via the <see cref="MessageReceived"/> event.</para>
        /// </remarks>
        ...
        public bool Connect(string hostAddress, int maxConnectionAttempts = 5, byte messageHandlerGroupId = 0, EmbeddedMessage message = null, bool useMessageHandlers = true)
        {
            Disconnect();
            ...
            {
                EmbeddedLogger.Log(LogType.Error, LogName, connectError);
            ...
            connectMessage = EmbeddedMessage.Create(MessageHeader.Connect);
            if (message != null)
            {
                if (message.ReadBits != 0)
                    EmbeddedLogger.Log(LogType.Error, LogName, $"Use the parameterless 'Message.Create()' overload when setting connection attempt data!");
            ...
            StartTime();
            Heartbeat();
            EmbeddedLogger.Log(LogType.Info, LogName, $"Connecting to {connection}...");
            return true;
        }
        ...
        protected override void CreateMessageHandlersDictionary(byte messageHandlerGroupId)
        {
            ...
            foreach (MethodInfo method in methods)
            {
                ...
                else
                {
                    ...
                    if (Delegate.CreateDelegate(typeof(EmbeddedServer.MessageHandler), method, false) == null)
        ...
        internal override void Heartbeat()
        {
            if (IsConnecting)
            {
                ...
                {
                    EmbeddedLogger.Log(LogType.Info, LogName, $"Trying to connect with server : attempt {connectionAttempts} of {maxConnectionAttempts}.");
        ...
        protected override void Handle(EmbeddedMessage message, MessageHeader header, EmbeddedConnection connection)
        {
            switch (header)
            {
                ...
                case MessageHeader.ClientDisconnected:
                    ...
                    break;
                default:
                    EmbeddedLogger.Log(LogType.Warning, LogName, $"Unexpected message header '{header}'! Discarding {message.BytesInUse} bytes.");
                    break;
            }
        ...
        public ushort Send(EmbeddedMessage message, bool shouldRelease = true) => connection.Send(message, shouldRelease);
        ...
        public void Disconnect()
        {
            ...
            Send(EmbeddedMessage.Create(MessageHeader.Disconnect));
        ...
        private void LocalDisconnect(DisconnectReason reason, EmbeddedMessage message = null, RejectReason rejectReason = RejectReason.NoConnection)
        {
            if (IsNotConnected)
                return;
        ...
        protected virtual void OnConnected()
        {
            ...
            connectMessage = null;
            EmbeddedLogger.Log(LogType.Info, LogName, "Connected successfully!");
        ...
        protected virtual void OnConnectionFailed(RejectReason reason, EmbeddedMessage message = null)
        {
            connectMessage.Release();
            connectMessage = null;
            EmbeddedLogger.Log(LogType.Info, LogName, $"Connection to server failed: {EmbeddedHelper.GetReasonString(reason)}.");
        ...
        protected virtual void OnMessageReceived(EmbeddedMessage message)
        {
        ...
        protected virtual void OnDisconnected(DisconnectReason reason, EmbeddedMessage message)
        {
            EmbeddedLogger.Log(LogType.Info, LogName, $"Disconnected from server: {EmbeddedHelper.GetReasonString(reason)}.");
        ...
        {
            EmbeddedLogger.Log(LogType.Info, LogName, $"Client {clientId} connected.");
        ...
        {
            EmbeddedLogger.Log(LogType.Info, LogName, $"Client {clientId} disconnected.");
