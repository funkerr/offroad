...
namespace com.onlineobject.objectnet.embedded {
    ...
    /// <summary>Represents a connection to a <see cref="EmbeddedServer"/> or <see cref="EmbeddedClient"/>.</summary>
    public abstract class EmbeddedConnection
    {
        ...
        public Action<EmbeddedMessage> NotifyReceived;
        ...
        public readonly EmbeddedConnectionMetrics Metrics;
        ...
        internal EmbeddedPeer Peer { get; private set; }
        ...
        private readonly Dictionary<ushort, EmbeddedPendingMessage> pendingMessages;
        ...
        {
            Metrics = new EmbeddedConnectionMetrics();
            ...
            pendingMessages = new Dictionary<ushort, EmbeddedPendingMessage>();
        }

        /// <summary>Initializes connection data.</summary>
        /// <param name="peer">The <see cref="Peer"/> which this connection belongs to.</param>
        /// <param name="timeoutTime">The timeout time.</param>
        internal void Initialize(EmbeddedPeer peer, int timeoutTime)
        {
            Peer = peer;
        ...
        ///   to <see langword="false"/>. <see cref="EmbeddedMessage.Release"/> can be used to manually return the message to the pool at a later time.
        /// </remarks>
        public ushort Send(EmbeddedMessage message, bool shouldRelease = true)
        {
            ushort sequenceId = 0;
            if (message.SendMode == MessageSendMode.Notify)
            {
                ...
                Buffer.BlockCopy(message.Data, 0, EmbeddedMessage.ByteBuffer, 0, byteAmount);
                Send(EmbeddedMessage.ByteBuffer, byteAmount);
            ...
            else
            {
                ...
                EmbeddedPendingMessage pendingMessage = EmbeddedPendingMessage.Create(sequenceId, message, this);
        ...
        internal void ProcessNotify(byte[] dataBuffer, int amount, EmbeddedMessage message)
        {
            notify.UpdateReceivedAcks(EmbeddedConverter.UShortFromBits(dataBuffer, EmbeddedMessage.HeaderBits), EmbeddedConverter.ByteFromBits(dataBuffer, EmbeddedMessage.HeaderBits + 16));

            Metrics.ReceivedNotify(amount);
            if (notify.ShouldHandle(EmbeddedConverter.UShortFromBits(dataBuffer, EmbeddedMessage.HeaderBits + 24)))
            {
        ...
        internal void LocalDisconnect()
        {
            ...
            foreach (EmbeddedPendingMessage pendingMessage in pendingMessages.Values)
                pendingMessage.Clear();

            pendingMessages.Clear();
        }

        /// <summary>Resends the <see cref="EmbeddedPendingMessage"/> with the given sequence ID.</summary>
        /// <param name="sequenceId">The sequence ID of the message to resend.</param>
        private void ResendMessage(ushort sequenceId)
        {
            if (pendingMessages.TryGetValue(sequenceId, out EmbeddedPendingMessage pendingMessage))
                pendingMessage.RetrySend();
        }
        
        /// <summary>Clears the <see cref="EmbeddedPendingMessage"/> with the given sequence ID.</summary>
        ...
        private void SendAck(ushort forSeqId, ushort lastReceivedSeqId, EmbeddedBitfield receivedSeqIds)
        {
            EmbeddedMessage message = EmbeddedMessage.Create(MessageHeader.Ack);
        ...
        internal void HandleAck(EmbeddedMessage message)
        {
        ...
        {
            EmbeddedMessage message = EmbeddedMessage.Create(MessageHeader.Welcome);
        ...
        internal bool HandleWelcomeResponse(EmbeddedMessage message)
        {
            if (!IsPending)
                return false;
            ...
            if (Id != id)
                EmbeddedLogger.Log(LogType.Error, Peer.LogName, $"Client has assumed ID {id} instead of {Id}!");
        ...
        internal void HandleHeartbeat(EmbeddedMessage message)
        {
            if (!IsConnected)
        ...
        {
            EmbeddedMessage message = EmbeddedMessage.Create(MessageHeader.Heartbeat);
        ...
        /// <param name="message">The welcome message to handle.</param>
        internal void HandleWelcome(EmbeddedMessage message)
        {
        ...
        /// <summary>Handles a heartbeat message.</summary>
        /// <param name="message">The heartbeat message to handle.</param>
        internal void HandleHeartbeatResponse(EmbeddedMessage message)
        {
        ...
        private abstract class Sequencer
        {
            ...
            protected readonly EmbeddedConnection connection;
            ...
            protected readonly EmbeddedBitfield receivedSeqIds = new EmbeddedBitfield();
            ...
            protected readonly EmbeddedBitfield ackedSeqIds = new EmbeddedBitfield(false);

            /// <summary>Initializes the sequencer.</summary>
            /// <param name="connection">The connection this sequencer belongs to.</param>
            protected Sequencer(EmbeddedConnection connection)
            {
        ...
        {
            /// <inheritdoc/>
            internal NotifySequencer(EmbeddedConnection connection) : base(connection) { }
            ...
            internal ushort InsertHeader(EmbeddedMessage message)
            {
                ushort sequenceId = NextSequenceId;
                ulong notifyBits = lastReceivedSeqId | ((ulong)receivedSeqIds.First8 << (2 * EmbeddedConverter.BitsPerByte)) | ((ulong)sequenceId << (3 * EmbeddedConverter.BitsPerByte));
                message.SetBits(notifyBits, 5 * EmbeddedConverter.BitsPerByte, EmbeddedMessage.HeaderBits);
                return sequenceId;
            }
            ...
            {
                int sequenceGap = EmbeddedHelper.GetSequenceGap(sequenceId, lastReceivedSeqId);
            ...
            {
                int sequenceGap = EmbeddedHelper.GetSequenceGap(remoteLastReceivedSeqId, lastAckedSeqId);
        ...
        {
            /// <inheritdoc/>
            internal ReliableSequencer(EmbeddedConnection connection) : base(connection) { }
            ...
            internal override bool ShouldHandle(ushort sequenceId)
            {
                ...
                if (sequenceGap != 0)
                {
                    ...
                    if (sequenceGap > 0)
                    {
                        // The received sequence ID is newer than the previous one
                        if (sequenceGap > 64)
                            EmbeddedLogger.Log(LogType.Warning, connection.Peer.LogName, $"The gap between received sequence IDs was very large ({sequenceGap})!");
