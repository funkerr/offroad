...
namespace com.onlineobject.objectnet.embedded {
    /// <summary>A server that can accept connections from <see cref="EmbeddedClient"/>s.</summary>
    public class EmbeddedServer : EmbeddedPeer
    {
        ...
        /// <summary>Sets the default timeout time for future connections and updates the <see cref="EmbeddedConnection.TimeoutTime"/> of all connected clients.</summary>
        public override int TimeoutTime
        {
            set
            {
                defaultTimeout = value;
                foreach (EmbeddedConnection connection in clients.Values)
        ...
        /// <remarks>The position of each <see cref="EmbeddedConnection"/> instance in the array does <i>not</i> correspond to that client's numeric ID (except by coincidence).</remarks>
        public EmbeddedConnection[] Clients => clients.Values.ToArray();
        ...
        /// <remarks>The <see cref="EmbeddedConnection"/> parameter is the pending connection and the <see cref="EmbeddedMessage"/> parameter is a message containing any additional data the
        /// client included with the connection attempt. If you choose to subscribe a method to this delegate, you should use it to call either <see cref="Accept(EmbeddedConnection)"/>
        /// or <see cref="Reject(EmbeddedConnection, EmbeddedMessage)"/>. Not doing so will result in the connection hanging until the client times out.</remarks>
        ...
        private IEmbeddedServer transport;
        ...
        /// <param name="logName">The name to use when logging messages via <see cref="EmbeddedLogger"/>.</param>
        public EmbeddedServer(IEmbeddedServer transport, string logName = "SERVER") : base(logName)
        {
            this.transport = transport;
            pendingConnections = new List<EmbeddedConnection>();
            clients = new Dictionary<ushort, EmbeddedConnection>();
            timedOutClients = new List<EmbeddedConnection>();
        }
        /// <summary>Handles initial setup using the built-in UDP transport.</summary>
        /// <param name="logName">The name to use when logging messages via <see cref="EmbeddedLogger"/>.</param>
        public EmbeddedServer(string logName = "SERVER") : this(new Transports.Udp.EmbeddedUdpServer(), logName) { }
        ...
        /// <remarks>Setting <paramref name="useMessageHandlers"/> to <see langword="false"/> will disable the automatic detection and execution of methods with the <see cref="EmbeddedMessageHandlerAttribute"/>, which is beneficial if you prefer to handle messages via the <see cref="MessageReceived"/> event.</remarks>
        public void Start(ushort port, ushort maxClientCount, byte messageHandlerGroupId = 0, bool useMessageHandlers = true) {
            this.InternalStart(port, maxClientCount, messageHandlerGroupId, useMessageHandlers);
        }
        ...
        /// <param name="useMessageHandlers">Whether or not the server should use the built-in message handler system.</param>
        /// <remarks>Setting <paramref name="useMessageHandlers"/> to <see langword="false"/> will disable the automatic detection and execution of methods with the <see cref="EmbeddedMessageHandlerAttribute"/>, which is beneficial if you prefer to handle messages via the <see cref="MessageReceived"/> event.</remarks>
        public void Start(string address, ushort port, ushort maxClientCount, byte messageHandlerGroupId = 0, bool useMessageHandlers = true) {
            this.InternalStart(port, maxClientCount, messageHandlerGroupId, useMessageHandlers, address);
        }
        ...
        /// <param name="useMessageHandlers">Whether or not the server should use the built-in message handler system.</param>
        /// <remarks>Setting <paramref name="useMessageHandlers"/> to <see langword="false"/> will disable the automatic detection and execution of methods with the <see cref="EmbeddedMessageHandlerAttribute"/>, which is beneficial if you prefer to handle messages via the <see cref="MessageReceived"/> event.</remarks>
        private void InternalStart(ushort port, ushort maxClientCount, byte messageHandlerGroupId = 0, bool useMessageHandlers = true, string address = null)
        {
            Stop();
            ...
            clients = new Dictionary<ushort, EmbeddedConnection>(maxClientCount);
            InitializeClientIds();
            ...
            Heartbeat();
            IsRunning = true;
            EmbeddedLogger.Log(LogType.Info, LogName, $"Started on port {port}.");
        }
        ...
        protected override void CreateMessageHandlersDictionary(byte messageHandlerGroupId)
        {
            ...
            {
                EmbeddedMessageHandlerAttribute attribute = method.GetCustomAttribute<EmbeddedMessageHandlerAttribute>();
                ...
                else
                {
                    ...
                    if (Delegate.CreateDelegate(typeof(EmbeddedClient.MessageHandler), method, false) == null)
        ...
        private void HandleConnect(EmbeddedConnection connection, EmbeddedMessage connectMessage)
        {
            ...
            else if (ClientCount < MaxClientCount)
            {
                if (!clients.ContainsValue(connection) && !pendingConnections.Contains(connection))
                {
                    ...
                    Send(EmbeddedMessage.Create(MessageHeader.Connect), connection); // Inform the client we've received the connection attempt
        ...
        public void Accept(EmbeddedConnection connection)
        {
            if (pendingConnections.Remove(connection))
                ...
                EmbeddedLogger.Log(LogType.Warning, LogName, $"Couldn't accept connection from {connection} because no such connection was pending!");
        }

        /// <summary>Rejects the given pending connection.</summary>
        /// <param name="connection">The connection to reject.</param>
        /// <param name="message">Data that should be sent to the client being rejected. Use <see cref="EmbeddedMessage.Create()"/> to get an empty message instance.</param>
        public void Reject(EmbeddedConnection connection, EmbeddedMessage message = null)
        {
            if (message != null && message.ReadBits != 0)
                EmbeddedLogger.Log(LogType.Error, LogName, $"Use the parameterless 'Message.Create()' overload when setting rejection data!");

            if (pendingConnections.Remove(connection))
                Reject(connection, message == null ? RejectReason.Rejected : RejectReason.Custom, message);
            else
                EmbeddedLogger.Log(LogType.Warning, LogName, $"Couldn't reject connection from {connection} because no such connection was pending!");
        }

        /// <summary>Accepts the given pending connection.</summary>
        /// <param name="connection">The connection to accept.</param>
        private void AcceptConnection(EmbeddedConnection connection)
        {
        ...
        private void Reject(EmbeddedConnection connection, RejectReason reason, EmbeddedMessage rejectMessage = null)
        {
            if (reason != RejectReason.AlreadyConnected)
            {
                ...
                EmbeddedMessage message = EmbeddedMessage.Create(MessageHeader.Reject);
            ...
            EmbeddedLogger.Log(LogType.Info, LogName, $"Rejected connection from {connection}: {EmbeddedHelper.GetReasonString(reason)}.");
        }
        ...
        internal override void Heartbeat()
        {
            ...
            foreach (EmbeddedConnection connection in pendingConnections)
                if (connection.HasConnectAttemptTimedOut)
                    timedOutClients.Add(connection);

            foreach (EmbeddedConnection connection in timedOutClients)
        ...
        /// <inheritdoc/>
        protected override void Handle(EmbeddedMessage message, MessageHeader header, EmbeddedConnection connection)
        {
            switch (header)
            {
                // User messages
                ...
                case MessageHeader.Welcome:
                    ...
                    break;
                default:
                    EmbeddedLogger.Log(LogType.Warning, LogName, $"Unexpected message header '{header}'! Discarding {message.BytesInUse} bytes received from {connection}.");
                    break;
            }
        ...
        /// <inheritdoc cref="EmbeddedConnection.Send(EmbeddedMessage, bool)"/>
        public void Send(EmbeddedMessage message, ushort toClient, bool shouldRelease = true)
        {
            if (clients.TryGetValue(toClient, out EmbeddedConnection connection))
        ...
        /// <param name="shouldRelease">Whether or not to return the message to the pool after it is sent.</param>
        /// <inheritdoc cref="EmbeddedConnection.Send(EmbeddedMessage, bool)"/>
        public ushort Send(EmbeddedMessage message, EmbeddedConnection toClient, bool shouldRelease = true) => toClient.Send(message, shouldRelease);

        /// <summary>Sends a message to all connected clients.</summary>
        /// <param name="message">The message to send.</param>
        /// <param name="shouldRelease">Whether or not to return the message to the pool after it is sent.</param>
        /// <inheritdoc cref="EmbeddedConnection.Send(EmbeddedMessage, bool)"/>
        public void SendToAll(EmbeddedMessage message, bool shouldRelease = true)
        {
            foreach (EmbeddedConnection client in clients.Values)
        ...
        /// <param name="shouldRelease">Whether or not to return the message to the pool after it is sent.</param>
        /// <inheritdoc cref="EmbeddedConnection.Send(EmbeddedMessage, bool)"/>
        public void SendToAll(EmbeddedMessage message, ushort exceptToClientId, bool shouldRelease = true)
        {
            foreach (EmbeddedConnection client in clients.Values)
        ...
        public bool TryGetClient(ushort id, out EmbeddedConnection client) => clients.TryGetValue(id, out client);

        /// <summary>Disconnects a specific client.</summary>
        /// <param name="id">The numeric ID of the client to disconnect.</param>
        /// <param name="message">Data that should be sent to the client being disconnected. Use <see cref="EmbeddedMessage.Create()"/> to get an empty message instance.</param>
        public void DisconnectClient(ushort id, EmbeddedMessage message = null)
        {
            if (message != null && message.ReadBits != 0)
                EmbeddedLogger.Log(LogType.Error, LogName, $"Use the parameterless 'Message.Create()' overload when setting disconnection data!");

            if (clients.TryGetValue(id, out EmbeddedConnection client))
            {
                SendDisconnect(client, DisconnectReason.Kicked, message);
                LocalDisconnect(client, DisconnectReason.Kicked);
            }
            else
                EmbeddedLogger.Log(LogType.Warning, LogName, $"Couldn't disconnect client {id} because it wasn't connected!");
        }

        /// <summary>Disconnects the given client.</summary>
        /// <param name="client">The client to disconnect.</param>
        /// <param name="message">Data that should be sent to the client being disconnected. Use <see cref="EmbeddedMessage.Create()"/> to get an empty message instance.</param>
        public void DisconnectClient(EmbeddedConnection client, EmbeddedMessage message = null)
        {
            if (message != null && message.ReadBits != 0)
                EmbeddedLogger.Log(LogType.Error, LogName, $"Use the parameterless 'Message.Create()' overload when setting disconnection data!");
            ...
            if (clients.ContainsKey(client.Id))
            {
                ...
                EmbeddedLogger.Log(LogType.Warning, LogName, $"Couldn't disconnect client {client.Id} because it wasn't connected!");
        }

        /// <inheritdoc/>
        internal override void Disconnect(EmbeddedConnection connection, DisconnectReason reason)
        {
        ...
        private void LocalDisconnect(EmbeddedConnection client, DisconnectReason reason)
        {
            if (client.Peer != this)
        ...
        public void Stop()
        {
            ...
            StopTime();
            IsRunning = false;
            EmbeddedLogger.Log(LogType.Info, LogName, "Server stopped.");
        }
        ...
        private ushort GetAvailableClientId()
        {
            ...
            EmbeddedLogger.Log(LogType.Error, LogName, "No available client IDs, assigned 0!");
            return 0;
        }
        ...
        private void SendDisconnect(EmbeddedConnection client, DisconnectReason reason, EmbeddedMessage disconnectMessage)
        {
            EmbeddedMessage message = EmbeddedMessage.Create(MessageHeader.Disconnect);
            message.AddByte((byte)reason);
        ...
        private void SendClientConnected(EmbeddedConnection newClient)
        {
            EmbeddedMessage message = EmbeddedMessage.Create(MessageHeader.ClientConnected);
        ...
        {
            EmbeddedMessage message = EmbeddedMessage.Create(MessageHeader.ClientDisconnected);
        ...
        protected virtual void OnClientConnected(EmbeddedConnection client)
        {
            EmbeddedLogger.Log(LogType.Info, LogName, $"Client {client.Id} ({client}) connected successfully!");
        ...
        protected virtual void OnConnectionFailed(EmbeddedConnection connection)
        {
            EmbeddedLogger.Log(LogType.Info, LogName, $"Client {connection} stopped responding before the connection was fully established!");
        ...
        protected virtual void OnMessageReceived(EmbeddedMessage message, EmbeddedConnection fromConnection)
        {
            ...
            if (useMessageHandlers)
            {
                ...
                else if (messageId != EmbeddedPeer.MESSAGE_PACKET)
                    EmbeddedLogger.Log(LogType.Warning, LogName, $"No message handler method found for message ID {messageId}!");
            }
        }
        ...
        protected virtual void OnClientDisconnected(EmbeddedConnection connection, DisconnectReason reason)
        {
            EmbeddedLogger.Log(LogType.Info, LogName, $"Client {connection.Id} ({connection}) disconnected: {EmbeddedHelper.GetReasonString(reason)}.");
