...
namespace com.onlineobject.objectnet.embedded {
    ...
    /// <summary>Provides base functionality for <see cref="EmbeddedServer"/> and <see cref="EmbeddedClient"/>.</summary>
    public abstract class EmbeddedPeer
    {
        /// <summary>The name to use when logging messages via <see cref="EmbeddedLogger"/>.</summary>
        public readonly string LogName;
        /// <summary>Sets the relevant connections' <see cref="EmbeddedConnection.TimeoutTime"/>s.</summary>
        ...
        /// <summary>The number of currently active <see cref="EmbeddedServer"/> and <see cref="EmbeddedClient"/> instances.</summary>
        ...
        private readonly EmbeddedPriorityQueue<DelayedEvent, long> eventQueue = new EmbeddedPriorityQueue<DelayedEvent, long>();

        // Constant representing the message packet type.
        public const ushort MESSAGE_PACKET = 9999;
        ...
        public EmbeddedPeer(string logName)
        {
            LogName = logName;
        }

        /// <summary>Retrieves methods marked with <see cref="EmbeddedMessageHandlerAttribute"/>.</summary>
        ...
        protected MethodInfo[] FindMessageHandlers()
        {
            ...
            return AppDomain.CurrentDomain.GetAssemblies()
                ...
                .Where(m => m.GetCustomAttributes(typeof(EmbeddedMessageHandlerAttribute), false).Length > 0)
                .ToArray();
        }
        ...
        {
            EmbeddedMessage message = EmbeddedMessage.Create().Init(e.DataBuffer[0], e.Amount, out MessageHeader header);
            
            if (message.SendMode == MessageSendMode.Notify)
            {
                if (e.Amount < EmbeddedMessage.MinNotifyBytes)
                    return;
            ...
            {
                if (e.Amount > EmbeddedMessage.MinUnreliableBytes)
            ...
            {
                if (e.Amount < EmbeddedMessage.MinReliableBytes)
                    return;

                e.FromConnection.Metrics.ReceivedReliable(e.Amount);
                if (e.FromConnection.ShouldHandle(EmbeddedConverter.UShortFromBits(e.DataBuffer, EmbeddedMessage.HeaderBits)))
                {
                    Buffer.BlockCopy(e.DataBuffer, 1, message.Data, 1, e.Amount - 1);
                    messagesToHandle.Enqueue(new MessageToHandle(message, header, e.FromConnection));
                }
        ...
        protected abstract void Handle(EmbeddedMessage message, MessageHeader header, EmbeddedConnection connection);
        ...
        internal abstract void Disconnect(EmbeddedConnection connection, DisconnectReason reason);

        /// <summary>Increases <see cref="ActiveCount"/>. For use when a new <see cref="EmbeddedServer"/> or <see cref="EmbeddedClient"/> is started.</summary>
        protected static void IncreaseActiveCount()
        {
            ActiveCount++;
        }

        /// <summary>Decreases <see cref="ActiveCount"/>. For use when a <see cref="EmbeddedServer"/> or <see cref="EmbeddedClient"/> is stopped.</summary>
    ...
    internal readonly struct MessageToHandle
    {
        ...
        internal readonly EmbeddedMessage Message;
        ...
        internal readonly EmbeddedConnection FromConnection;
        ...
        public MessageToHandle(EmbeddedMessage message, MessageHeader header, EmbeddedConnection fromConnection)
        {
            Message = message;
            Header = header;
